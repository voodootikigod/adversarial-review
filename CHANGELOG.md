...
> Entries below accumulated across the 2.1–2.9 releases and were never cut into version
...
### Added
- **Windows support.** The review and the full test suite now run on Windows. Spawns route through the safe interpreter path rather than handing a bare command name to `cmd.exe` for re-parsing, so an attacker-placed executable earlier on `PATH` can no longer be resolved in place of the intended CLI.
...
- **Bubblewrap confinement for Linux fixers**: fixer processes run under `bwrap` with host and secret paths confined, so a fix pass cannot reach credentials or system paths outside the working tree.
...
### Fixed
- **Out-of-diff repository code is no longer reported as fabricated evidence.** Grounding previously searched only the diff, so a finding that correctly quoted a real file outside the change set was penalised exactly like an invented quote. Haystacks are now route-aware, paths are resolved against the repo root with containment enforced, and trust-root discovery is isolated so a security error is never swallowed. Fabricated evidence is still caught. (#47, #30)
- **`--provider agy` now honours the review budget.** agy's print mode enforces its own `--print-timeout` (default 5m), so any review longer than five minutes was abandoned with `Error: timeout waiting for response` regardless of the `--timeout` requested. The resolved budget is forwarded to agy, its timeout is reported as a `--timeout` overrun instead of an opaque exit 1, and a single source for the duration means the flag sent and the message shown can no longer disagree. Degenerate budgets no longer render as `NaNs`/`0s`. (#43)
