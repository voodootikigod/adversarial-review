...
function runCli(args, { mocks = {}, env = {}, afterRun } = {}) {
  ...
  try {
    ...
    const mockNodePath = path.join(mocksDir, nodeExeName);
    try {
      fs.symlinkSync(process.execPath, mockNodePath);
    } catch {
      fs.copyFileSync(process.execPath, mockNodePath);
    }

    for (const [name, body] of Object.entries(mocks)) {
      writeMockBin(mocksDir, name, `
import fs from "node:fs";
...
`);
    ...
    const PATH = [mocksDir, ...sysDirs].join(path.delimiter);
...
test("CLI: multi-provider needs-attention maps to exit 2 (both providers reachable)", () => {
  const r = runCli(["--providers", "claude,openai", "--scope", "working-tree", "--allow-secrets"], {
    mocks: { claude: FLAG, codex: APPROVE }
  });
...
test("CLI: under-satisfied emits a loud notice and proceeds (AC7 end-to-end)", () => {
  const r = runCli(["--providers", "claude,openai", "--scope", "working-tree", "--allow-secrets"], {
    mocks: { claude: FLAG } // codex absent, no keys → openai unreachable
  });
...
test("CLI #2: --json verdict matches the quorum exit verdict (no contradiction)", () => {
  ...
  const r = runCli(
    ["--providers", "claude,openai", "--quorum", "2", "--json", "--scope", "working-tree", "--allow-secrets"],
    { mocks: { claude: FLAG, codex: APPROVE } }
  );
...
test("CLI #2(r5): when quorum is not met, the JSON summary matches the approve verdict", () => {
  // claude flags, openai approves, --quorum 2 → effective quorum 2 not met → approve.
...
test("CLI #2(r7): a needs-attention verdict never carries an approving summary", () => {
  const r = runCli(
    ["--providers", "claude,openai", "--json", "--scope", "working-tree", "--allow-secrets"],
    { mocks: { claude: FLAG, codex: APPROVE } }
  );
  assert.equal(r.status, 2, r.stderr);
  const out = JSON.parse(r.stdout);
...
const UNGROUNDED_FLAG = '{"verdict":"needs-attention","summary":"bad","coverage":{"files_examined":["code.js"],"files_skipped":[]},"findings":[{"severity":"high","category":"security","title":"t","body":"b","exploit_scenario":"e","evidence":"not in diff at all","file":"code.js","line_start":1,"line_end":1,"confidence":1.0,"recommendation":"r"}],"next_steps":["n"]}';

test("CLI #1(r7): a non-inlinable API provider falls back to its local CLI instead of being dropped", () => {
  const provider = process.platform === "win32" ? "openai" : "gemini";
  const cliCmd = process.platform === "win32" ? "codex" : "agy";
  const envKey = process.platform === "win32" ? "OPENAI_API_KEY" : "GEMINI_API_KEY";
  const r = runCli(
    ["--providers", provider, "--scope", "working-tree", "--allow-secrets", "--max-bytes", "1"],
    { mocks: { [cliCmd]: FLAG }, env: { [envKey]: "dummy" } }
  );
  assert.match(r.stderr, new RegExp(`using local ${cliCmd}`), `API ${provider} downgrades to the ${cliCmd} CLI`);
  assert.equal(r.status, 2, `${cliCmd} reviews the summary-mode diff and flags → exit 2`);
});
...
test("CLI #6: an API provider on a non-inlinable diff is dropped, CLI provider proceeds", () => {
  ...
  assert.match(r.stderr, /Dropping 1 API provider\(s\)/);
...
test("CLI #5: grounding warnings reach stderr even with --json", () => {
  const r = runCli(
    ["--providers", "claude", "--json", "--scope", "working-tree", "--allow-secrets"],
    { mocks: { claude: UNGROUNDED_FLAG } }
  );
  assert.equal(r.status, 2);
  assert.match(r.stderr, /quoted evidence was not found/);
  const out = JSON.parse(r.stdout);
  assert.equal(out.verdict, "needs-attention");
});
...
test("CLI: no requested provider reachable exits 1 (operational error, not a verdict)", () => {
  const provider = process.platform === "win32" ? "openai" : "gemini";
  const r = runCli(
    ["--providers", provider, "--scope", "working-tree", "--allow-secrets"],
    { mocks: {} }
  );
  assert.equal(r.status, 1);
  assert.match(r.stderr, /None of the requested providers are reachable|No usable providers/);
});
...
test("CLI #6: all-API selection on a non-inlinable diff exits 1 (nothing usable)", () => {
  const r = runCli(
    ...
    { mocks: {}, env: { GEMINI_API_KEY: "dummy" } }
  );
  assert.equal(r.status, 1);
...
test("ledger AC3: without --findings-ledger, no .adlc/ is created and no ledger warning", () => {
  const r = runCli(
    ["--providers", "claude", "--scope", "working-tree", "--allow-secrets"],
    { mocks: { claude: FLAG }, afterRun: (repoDir) => {
      assert.equal(fs.existsSync(path.join(repoDir, ".adlc")), false);
    }}
  );
  assert.equal(r.status, 2);
  assert.doesNotMatch(r.stderr, /findings ledger/i);
});
...
test("ledger: multi-provider does NOT record an ungrounded corroborated finding (negative grounding)", () => {
  ...
  try {
    const r = runCli(
      ["--providers", "claude,openai", "--scope", "working-tree", "--allow-secrets", "--findings-ledger", ledger],
      { mocks: { claude: UNGROUNDED, codex: UNGROUNDED } }
    );
...
test("ledger: records the MERGED findings (shared corroborated once + each unique), R2", () => {
  // claude: shared + onlyA ; codex: shared + onlyB. Merged = 3 distinct gating
  ...
  const codexOut = res([f("code.js", "Shared issue", "security", 1), f("code.js", "Only in B", "correctness", 20)].join(","));
  ...
  try {
    runCli(
      ["--providers", "claude,openai", "--scope", "working-tree", "--allow-secrets", "--findings-ledger", ledger],
      { mocks: { claude: claudeOut, codex: codexOut } }
    );
